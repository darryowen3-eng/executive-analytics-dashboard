...
# Configure the look and title of your browser dashboard page
...
# ==========================================
# 2. DATA EXTRACTION LAYER (SQL Queries)
# ==========================================
try:
    # Query our Step 1 retail transaction records
    orders_df = pd.read_sql("SELECT * FROM fct_orders", con=engine)
    customers_df = pd.read_sql("SELECT * FROM dim_customers", con=engine)
    
    # Query our Step 2 live currency metrics
    ...
    # ==========================================
    # Layout Grid: Split the screen into two large columns side-by-side
    col1, col2 = st.columns(2)

    with col1:
        st.subheader("🛍️ Total Sales Revenue by Item")
    ...
    with col2:
        ...
        # Build an interactive Plotly horizontal bar chart for currency indicators
